...
impl Henk {
    pub fn type_check(self) -> Result<Henk, String> {
        ...
        let mut context: HashMap<String, Henk> = HashMap::new();
        self.type_check_with_context(&mut context)
    }

    pub fn type_check_with_context(self, context: &mut HashMap<String, Henk>) -> Result<Henk, String> {
        ...
        match self {
            ...
            Henk::Application(left, right) => {
                let left_ty = left.type_check_with_context(context)?;
                match left_ty.whnf() {
                    Henk::Lambda(bound, ty_in, ty_out) => {
                        let right_ty = right.clone().type_check_with_context(context)?;
            ...
            Henk::Lambda(bound, ty, inner) => {
                ty.clone().type_check_with_context(context)?;
                // let mut new_context = context;
                context.insert(bound.clone(), *ty.clone());
                let inner_type = inner.type_check_with_context(context)?;
                Ok(Henk::Forall(bound, ty, Box::new(inner_type)))
            }
            Henk::Forall(bound, left, right) => {
                let left_sort = left
                    .clone()
                    .type_check_with_context(context)
                    .map(Henk::whnf)?;
                context.insert(bound.clone(), *left.clone());
    ...
    pub fn free_vars(&self) -> HashSet<&String> {
        ...
        match *self {
            ...
            Henk::Lambda(ref bound, ref ty, ref inner) => {
                closure_vars(bound, ty, inner);
            }
            Henk::Forall(ref bound, ref left, ref right) => {
                closure_vars(bound, left, right);
            }
        }
        set
    }
    ...
    pub fn substitute(self, from: &String, to: &Henk) -> Henk {
        match self {
            ...
            Henk::Application(left, right) => Henk::Application(
                        ...
                        loop {
                            ...
                            if used.contains(&unused) {
                                ...
                                let renamed = Henk::Lambda(
                                    ...
                                    Box::new(
                                        ty.clone()
                                            .substitute(bound, &Henk::Variable(unused.clone())),
                                    ),
                                    Box::new(
                                        inner.clone().substitute(bound, &&Henk::Variable(unused)),
                                    ),
                                );
                        ...
                        loop {
                            ...
                            if used.contains(&unused) {
                                ...
                                let renamed = Henk::Forall(
                                    ...
                                    Box::new(
                                        right.clone().substitute(bound, &&Henk::Variable(unused)),
                                    ),
                                );
                                return renamed.substitute(from, to);
                            }
        ...
        fn spine(leftmost: Henk, stack: &[Henk]) -> Henk {
            match (leftmost, stack) {
                ...
                (Henk::Lambda(ref from, _, ref inner), ref stack) if !stack.is_empty() => {
                    let mut new_stack: Vec<Henk> = (*stack).into();
                    let right = new_stack.last().unwrap();
                    spine(inner.clone().substitute(&from, &right), &new_stack)
                }
                (leftmost, _) => stack.iter().fold(leftmost, |l, r| {
                    Henk::Application(Box::new(l), Box::new(r.clone()))
                }),
            }
        }
        spine(self, &[])
    ...
    pub fn nf(self) -> Henk {
        fn spine(leftmost: Henk, stack: &[Henk]) -> Henk {
            match (leftmost, stack) {
                ...
                (leftmost, _) => stack.iter().fold(leftmost, |l, r| {
                    Henk::Application(Box::new(l), Box::new(r.clone().nf()))
                }),
            }
        }
        spine(self, &[])
